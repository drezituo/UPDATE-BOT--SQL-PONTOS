...
async def obter_valor_jogador_embed(guild: discord.Guild, nome_jogador: str, membro: discord.Member = None):
    """Mostra a menção do jogador e mantém o número visível entre parênteses.
    O número vem primeiro da base de dados; se falhar, tenta extrair do nome/nickname.
    """
    def numero_do_nome(texto: str):
        match = re.search(r"#(\d+)", str(texto or ""))
        if not match:
            return None
        try:
    ...
    def formatar_valor(member: discord.Member = None, fallback_nome: str = "") -> str:
        numero = obter_numero_jogador_sync(member.id) if member is not None else None

        if numero is None and member is not None:
            numero = numero_do_nome(member.display_name) or numero_do_nome(member.name)

        if numero is None:
            numero = numero_do_nome(fallback_nome)

        base = member.mention if member is not None else (normalizar_nome(fallback_nome) or "Jogador")
    ...
    if membro is not None:
        return formatar_valor(membro, nome_jogador)

    membro_encontrado = await encontrar_membro_por_nome(guild, nome_jogador)
    if membro_encontrado:
        return formatar_valor(membro_encontrado, nome_jogador)

    return formatar_valor(None, nome_jogador)
...
if conn:
    ...
    try:
        ...
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS termos_uploads (
            id BIGSERIAL PRIMARY KEY,
            user_id BIGINT NOT NULL,
...
class InscricoesView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="inscricoes:enviar_termo"
    )
    async def botao_enviar_termo(self, interaction: discord.Interaction, button: discord.ui.Button):
        await enviar_termo_interaction(interaction)


class TermoAprovacaoView(discord.ui.View):
    def __init__(self, upload_id: int, user_id: int, bloqueado: bool = False):
        super().__init__(timeout=None)
    ...
    async def _staff_only(self, interaction: discord.Interaction) -> bool:
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("❌ Apenas staff/admin pode rever termos.", ephemeral=True)
            return False
        return True

    async def _notificar_jogador(self, interaction: discord.Interaction, mensagem: str):
        try:
    ...
    @discord.ui.button(label="Aprovar", emoji="✅", style=discord.ButtonStyle.green, custom_id="termo:aprovar")
    async def aprovar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            cursor.execute("""
                UPDATE termos_uploads
                SET estado = 'aprovado', revisto_por = %s, revisto_em = NOW()
                WHERE id = %s AND estado = 'pendente'
                RETURNING user_id, file_name
            """, (interaction.user.id, self.upload_id))
            row = cursor.fetchone()
            ...
            if not row:
                ...
                cursor.close()
                conn.close()
                return await interaction.response.send_message("⚠️ Este termo já foi revisto ou não existe.", ephemeral=True)
            ...
            cursor.close()
            conn.close()
            print(f"Erro ao aprovar termo: {e}")
            return await interaction.response.send_message("⚠️ Erro ao aprovar o termo.", ephemeral=True)
        finally:
            try:
                cursor.close()
                conn.close()
        ...
        embed = interaction.message.embeds[0] if interaction.message.embeds else discord.Embed(title="📄 Termo")
        embed.color = discord.Color.green()
        embed.add_field(name="✅ Estado", value=f"Aprovado por {interaction.user.mention}", inline=False)
    ...
    async def rejeitar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            cursor.execute("""
                UPDATE termos_uploads
                SET estado = 'rejeitado', revisto_por = %s, revisto_em = NOW()
                WHERE id = %s AND estado = 'pendente'
                RETURNING user_id, file_name
            """, (interaction.user.id, self.upload_id))
            row = cursor.fetchone()
            ...
            cursor.close()
            conn.close()
            print(f"Erro ao rejeitar termo: {e}")
            return await interaction.response.send_message("⚠️ Erro ao rejeitar o termo.", ephemeral=True)
        finally:
            try:
                cursor.close()
                conn.close()
        ...
        embed.add_field(name="❌ Estado", value=f"Rejeitado por {interaction.user.mention}", inline=False)

        await interaction.message.edit(embed=embed, view=TermoAprovacaoView(self.upload_id, self.user_id, bloqueado=True))
        await self._notificar_jogador(
            interaction,
            "❌ O teu termo de responsabilidade foi rejeitado. Por favor envia um novo ficheiro através do botão 📄 Enviar termo."
        )
        await interaction.response.send_message("❌ Termo rejeitado.", ephemeral=True)


async def carregar_views_termos_pendentes():
    """Reativa os botões Aprovar/Rejeitar de termos pendentes depois de reiniciar o bot."""
    conn = get_connection()
    if not conn:
        return
    ...
    try:
        cursor.execute("""
            SELECT id, user_id, staff_message_id
            FROM termos_uploads
            WHERE estado = 'pendente'
              AND staff_message_id IS NOT NULL
        """)
        for upload_id, user_id, staff_message_id in cursor.fetchall():
            bot.add_view(TermoAprovacaoView(upload_id, user_id), message_id=staff_message_id)
    except Exception as e:
        print(f"Erro ao carregar views de termos pendentes: {e}")
    finally:
        cursor.close()
        conn.close()


async def enviar_termo_interaction(interaction: discord.Interaction):
    if TERMOS_CHANNEL_ID == 0:
        return await interaction.response.send_message(
            "⚠️ O canal da staff para termos ainda não está configurado.",
            ephemeral=True
        )
    ...
    if canal_staff is None:
        return await interaction.response.send_message(
            "⚠️ Não consegui encontrar o canal da staff para receber termos.",
            ephemeral=True
        )
    ...
    try:
        ...
        return await interaction.response.send_message(
            "⚠️ Não consegui enviar-te DM. Ativa as mensagens privadas deste servidor e tenta novamente.",
            ephemeral=True
        )
...
async def processar_termo_recebido_dm(message: discord.Message) -> bool:
    ...
    if conn:
        ...
        try:
            ...
            conn.rollback()
            print(f"Erro ao guardar mensagem de staff do termo: {e}")
        finally:
            cursor.close()
            conn.close()
...
async def criar_inscricao_interaction(interaction: discord.Interaction, numero_texto: str = None):
    ...
    if not is_admin and not termo_assinado_sync(membro_jogador.id):
        return await interaction.response.send_message(
...
def criar_embed_painel_equipas():
    ...
    embed.set_footer(text="Os status são privados; usa o botão Partilhar para os publicar nesta sala")
    return embed
...
@bot.command(name="painel", aliases=["painel_equipas"])
@commands.has_permissions(administrator=True)
async def comando_painel_equipas(ctx, tipo: str = "equipas"):
    """Cria ou recria o painel de equipas.

    Aceita tanto ``!painel equipas`` como ``!painel_equipas``.
    """
    tipo_normalizado = normalizar_nome(tipo).lower().replace("_", " ")
    if tipo_normalizado not in {"equipa", "equipas"}:
        return await ctx.send(
            "⚠️ Usa `!painel equipas` ou `!painel_equipas`.",
            delete_after=10
        )
    ...
    if canal is None:
        return await ctx.send(
            f"❌ Não consegui encontrar o canal configurado para o painel (`{EQUIPAS_PANEL_CHANNEL_ID}`).",
            delete_after=15
        )

    await ctx.send(
        f"✅ Painel de equipas criado novamente em {canal.mention}.",
        delete_after=10
    )
...
class PartilharStatusJogadorView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "❌ Apenas o jogador que abriu este status o pode partilhar.",
                ephemeral=True
            )
            return False
        return True
    ...
    async def partilhar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            pass
        await interaction.followup.send("✅ O teu status foi partilhado nesta sala.", ephemeral=True)


class GestaoEquipaView(discord.ui.View):
    def __init__(self, equipa_row, utilizador_id: int):
        super().__init__(timeout=300)
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.utilizador_id:
            await interaction.response.send_message(
                "❌ Estes botões pertencem ao jogador que abriu este status.",
                ephemeral=True
            )
            return False
        return True

    @discord.ui.button(label="Partilhar", emoji="📤", style=discord.ButtonStyle.success, row=1)
    async def partilhar(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        equipa_row = obter_equipa_do_utilizador_sync(interaction.user.id)
        if not equipa_row or equipa_row[0] != self.equipa_row[0]:
            await interaction.followup.send("⚠️ Já não pertences a esta equipa.", ephemeral=True)
            return
        ...
        try:
            ...
            pass
        await interaction.followup.send("✅ O status da equipa foi partilhado nesta sala.", ephemeral=True)
...
class PainelEquipasView(discord.ui.View):
    ...
    async def status(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Resposta privada para evitar spam; o jogador decide se quer partilhar.
        await interaction.response.defer(ephemeral=True)
        ...
        await interaction.followup.send(
            embed=embed,
            view=PartilharStatusJogadorView(interaction.user.id),
            ephemeral=True
        )
...
class NFCFakeMessage:
    """Mensagem mínima para comandos que precisam de msg.id quando executados via NFC."""
...
class NFCFakeCtx:
    ...
    async def send(self, content=None, *, embed=None, delete_after=None, view=None):
        ...
        # Quando um comando real precisa de msg.id (ex.: BUNKER-551), tentamos enviar
        # para o canal real da equipa. Se falhar, devolvemos uma mensagem fake para não rebentar.
        canal = bot.get_channel(self.channel.id)
        if canal is not None:
            try:
                return await canal.send(content=content, embed=embed, delete_after=delete_after, view=view)
            except (discord.Forbidden, discord.HTTPException):
                pass

        return NFCFakeMessage(int(datetime.now(timezone.utc).timestamp() * 1000))
...
def _nfc_hidden_payload(team: str, codigo_lido: str, action: str) -> str:
    return (
        f'<input type="hidden" name="team" value="{escape(str(team or ""))}">'
        f'<input type="hidden" name="codigo" value="{escape(str(codigo_lido or ""))}">'
        f'<input type="hidden" name="action" value="{escape(str(action or "codigo"))}">'
        f'<input type="hidden" name="token" value="{escape(NFC_API_TOKEN)}">'
    )


def _nfc_confirm_html(team: str, codigo_lido: str, action: str, status: int = 200):
    """Página intermédia: NFC detetado, mas ainda sem validar no Discord."""
    ...
    operation_state = "ATIVA" if active else "INATIVA"
    payload = _nfc_hidden_payload(team_raw, codigo_lido, action_raw)
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>DUALITY SATCOM - CONFIRMAR</title>
  <style>
    :root {{ color-scheme: dark; }}
    * {{ box-sizing: border-box; }}
    body {{ margin:0; min-height:100dvh; display:grid; place-items:center; padding:10px; background:radial-gradient(circle at 50% 20%, rgba(14,165,233,.20), transparent 34%), linear-gradient(180deg,#020617,#07111f 48%,#020617); color:#dbeafe; font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Courier New",monospace; }}
    body:before {{ content:""; position:fixed; inset:0; pointer-events:none; background:repeating-linear-gradient(to bottom, rgba(148,163,184,.08) 0, rgba(148,163,184,.08) 1px, transparent 1px, transparent 4px); opacity:.34; }}
    .terminal {{ position:relative; z-index:1; width:min(94vw,760px); border:1px solid rgba(125,211,252,.36); border-radius:22px; background:rgba(2,6,23,.9); box-shadow:0 24px 90px rgba(0,0,0,.55),0 0 45px rgba(14,165,233,.12); overflow:hidden; }}
    .topbar {{ display:flex; justify-content:space-between; gap:12px; padding:15px 18px; border-bottom:1px solid rgba(125,211,252,.22); background:linear-gradient(90deg,rgba(8,47,73,.72),rgba(15,23,42,.65)); color:#93c5fd; font-size:12px; letter-spacing:.12em; }}
    .content {{ padding:26px; }}
    .stamp {{ display:inline-flex; padding:9px 12px; border:1px solid #38bdf8; border-radius:999px; background:rgba(56,189,248,.14); color:#7dd3fc; font-weight:800; font-size:12px; letter-spacing:.08em; }}
    ...
    .label {{ color:#64748b; font-size:11px; letter-spacing:.12em; text-transform:uppercase; margin-bottom:7px; }}
    .value {{ color:#e0f2fe; font-weight:800; font-size:15px; }}
    .message {{ margin-top:18px; padding:16px; border-left:3px solid #38bdf8; background:linear-gradient(90deg,rgba(56,189,248,.13),rgba(15,23,42,.45)); border-radius:14px; color:#e5e7eb; line-height:1.45; font-family:Arial,sans-serif; font-size:16px; }}
    button {{ width:100%; margin-top:18px; padding:18px 16px; border:0; border-radius:16px; background:linear-gradient(90deg,#0ea5e9,#22c55e); color:#020617; font-weight:900; letter-spacing:.12em; font-size:16px; cursor:pointer; box-shadow:0 0 30px rgba(34,197,94,.22); }}
    button:active {{ transform:scale(.99); }}
    .classified {{ margin-top:14px; color:#64748b; font-size:12px; letter-spacing:.08em; text-transform:uppercase; }}
    @media (max-width:640px) {{ .terminal{{width:100%; border-radius:18px}} .content{{padding:18px 16px}} .topbar{{font-size:10px; flex-wrap:wrap; padding:13px 14px}} .grid{{gap:9px}} .cell{{padding:11px 10px; min-height:70px}} .label{{font-size:9px}} .value{{font-size:13px}} h1{{font-size:clamp(34px,11vw,48px)}} button{{padding:17px 14px; font-size:14px}} }}
  </style>
</head>
<body>
  <main class="terminal">
    <div class="topbar"><div>DUALITY // SATCOM TERMINAL</div><div>NFC DETETADO</div></div>
    <section class="content">
      <div class="stamp">AGUARDA CONFIRMAÇÃO MANUAL</div>
      <h1>VALIDAR?</h1>
      <div class="subtitle">O CÓDIGO CONTINUA CLASSIFICADO - NADA FOI EXECUTADO</div>
      <div class="grid">
        <div class="cell"><div class="label">Operação</div><div class="value">{operation_state}</div></div>
      ...
      <div class="message">Confirma apenas se a equipa está fisicamente no objetivo e autorizada a validar esta ação.</div>
      <form method="post" action="/api/nfc/milsim/confirm">
        {payload}
        <button type="submit">VALIDAR OPERAÇÃO</button>
      </form>
      <div class="classified">Tokens e códigos operacionais ocultados da interface.</div>
    </section>
  </main>
</body>
</html>"""
...
def _nfc_bunker_bar_html(team: str, codigo_lido: str, action: str, status: int = 200):
    """Mini-game da barra para BUNKER-551. Só valida depois de sincronizar."""
    team_raw = str(team or "azul").strip().lower()
    action_raw = str(action or "codigo").strip().lower()
    payload = _nfc_hidden_payload(team_raw, codigo_lido, action_raw)
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>BUNKER UPLINK - SATCOM</title>
  <style>
    :root {{ color-scheme: dark; }} * {{ box-sizing:border-box; }}
    body {{ margin:0; min-height:100dvh; display:grid; place-items:center; padding:12px; background:radial-gradient(circle at 50% 18%,rgba(34,211,238,.20),transparent 34%),linear-gradient(180deg,#020617,#07111f 52%,#020617); color:#dbeafe; font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Courier New",monospace; overflow:hidden; }}
    body:before {{ content:""; position:fixed; inset:0; pointer-events:none; background:repeating-linear-gradient(to bottom,rgba(148,163,184,.08) 0,rgba(148,163,184,.08) 1px,transparent 1px,transparent 4px); opacity:.36; }}
    .terminal {{ width:min(94vw,680px); border:1px solid rgba(125,211,252,.38); border-radius:22px; background:rgba(2,6,23,.91); box-shadow:0 24px 90px rgba(0,0,0,.58),0 0 55px rgba(14,165,233,.14); overflow:hidden; position:relative; z-index:1; }}
    .topbar {{ display:flex; justify-content:space-between; gap:12px; padding:15px 18px; border-bottom:1px solid rgba(125,211,252,.22); background:linear-gradient(90deg,rgba(8,47,73,.78),rgba(15,23,42,.68)); color:#93c5fd; font-size:12px; letter-spacing:.12em; }}
    .content {{ padding:26px; }}
    .stamp {{ display:inline-flex; padding:9px 12px; border:1px solid #38bdf8; border-radius:999px; background:rgba(56,189,248,.14); color:#7dd3fc; font-weight:800; font-size:12px; letter-spacing:.08em; }}
    h1 {{ margin:18px 0 6px; color:#f8fafc; font-size:clamp(34px,9vw,60px); line-height:.92; letter-spacing:-.07em; text-transform:uppercase; }}
    .subtitle {{ color:#7dd3fc; letter-spacing:.14em; font-size:12px; margin-bottom:20px; }}
    .panel {{ padding:18px; border:1px solid rgba(125,211,252,.18); border-radius:18px; background:rgba(15,23,42,.72); }}
    .bar {{ position:relative; width:100%; height:34px; border-radius:999px; overflow:hidden; background:#020617; border:1px solid rgba(125,211,252,.26); box-shadow:inset 0 0 22px rgba(0,0,0,.7); }}
    .zone {{ position:absolute; top:0; bottom:0; left:42%; width:16%; background:rgba(34,197,94,.42); box-shadow:0 0 22px rgba(34,197,94,.5); }}
    .cursor {{ position:absolute; top:-6px; bottom:-6px; width:5px; border-radius:999px; background:#f8fafc; box-shadow:0 0 18px #38bdf8; left:0%; animation:sweep 1.55s linear infinite alternate; }}
    @keyframes sweep {{ from {{ left:0%; }} to {{ left:calc(100% - 5px); }} }}
    .readout {{ display:flex; justify-content:space-between; gap:10px; margin-top:13px; color:#94a3b8; font-size:12px; letter-spacing:.08em; }}
    .msg {{ margin-top:16px; min-height:42px; padding:14px; border-left:3px solid #38bdf8; border-radius:12px; background:rgba(56,189,248,.10); font-family:Arial,sans-serif; color:#e5e7eb; line-height:1.35; }}
    button {{ width:100%; margin-top:18px; padding:18px 16px; border:0; border-radius:16px; background:linear-gradient(90deg,#0ea5e9,#22c55e); color:#020617; font-weight:900; letter-spacing:.12em; font-size:16px; cursor:pointer; }}
    button.fail {{ background:linear-gradient(90deg,#ef4444,#f97316); }}
    form {{ display:none; }}
    @media (max-width:640px) {{ .terminal{{width:100%; border-radius:18px}} .content{{padding:18px 16px}} .topbar{{font-size:10px; flex-wrap:wrap; padding:13px 14px}} h1{{font-size:clamp(36px,12vw,52px)}} .panel{{padding:15px}} button{{padding:17px 14px; font-size:14px}} }}
  </style>
</head>
<body>
  <main class="terminal">
    <div class="topbar"><div>DUALITY // BUNKER TERMINAL</div><div>SIGNAL LOCK</div></div>
    <section class="content">
      <div class="stamp">SATCOM UPLINK</div>
      <h1>SINCRONIZAR</h1>
      <div class="subtitle">ALINHA A FREQUÊNCIA NA JANELA VERDE</div>
      <div class="panel">
        <div class="bar" id="bar"><div class="zone"></div><div class="cursor" id="cursor"></div></div>
        <div class="readout"><span>FREQ LOCK</span><span id="pct">--%</span></div>
        <div class="msg" id="msg">Aguarda alinhamento e pressiona sincronizar no momento certo.</div>
        <button id="sync" type="button">SINCRONIZAR UPLINK</button>
        <form id="confirm" method="post" action="/api/nfc/milsim/confirm">{payload}</form>
      </div>
    </section>
  </main>
  <script>
    ...
    let locked = false;
    function cursorPercent() {{
      const bar = document.getElementById('bar').getBoundingClientRect();
      const cur = cursor.getBoundingClientRect();
      const center = cur.left + cur.width/2 - bar.left;
      return Math.max(0, Math.min(100, (center / bar.width) * 100));
    }}
    setInterval(() => {{ if (!locked) pct.textContent = Math.round(cursorPercent()) + '%'; }}, 80);
    btn.addEventListener('click', () => {{
      if (locked) return;
      const p = cursorPercent();
      if (p >= 42 && p <= 58) {{
        locked = true;
        cursor.style.animationPlayState = 'paused';
        msg.textContent = 'SIGNAL LOCK confirmado. A transmitir validação para o Comando Central...';
        btn.textContent = 'UPLINK AUTORIZADO';
        setTimeout(() => form.submit(), 450);
      }} else {{
        msg.textContent = 'SIGNAL LOST. Frequência fora da janela. Tenta novamente.';
        btn.classList.add('fail');
        setTimeout(() => {{ btn.classList.remove('fail'); msg.textContent = 'Recalibra a frequência e pressiona sincronizar na zona verde.'; }}, 900);
      }}
    }});
  </script>
</body>
...
async def nfc_milsim_tap(request: web.Request):
    ...
    team = str(request.query.get("team", "")).strip().lower()
    codigo_lido = str(request.query.get("codigo", "")).strip().upper()
    action = str(request.query.get("action", "codigo")).strip().lower()

    # GET da tag NFC nunca executa diretamente. Primeiro mostra confirmação.
    # Exceção visual: BUNKER-551 mostra o mini-game da barra antes de confirmar.
    if action == "codigo" and codigo_lido == "BUNKER-551":
        return _nfc_bunker_bar_html(team, codigo_lido, action)

    return _nfc_confirm_html(team, codigo_lido, action)


async def nfc_milsim_confirm(request: web.Request):
    try:
        data = await request.post()
    except Exception:
        return _nfc_html(False, "Pedido inválido.", status=400)

    # Confirmação por botão: aceita o token enviado pelo formulário interno.
...
async def start_nfc_api_server():
    ...
    app_api.router.add_post("/api/nfc/milsim/confirm", nfc_milsim_confirm)
...
class EscolhaCancelamentoValidadoView(discord.ui.View):
    def __init__(self, inscricao_id, message_id, user_id, nome_jogador, inscrito_por, thread_id):
        ...
        self.inscrito_por = inscrito_por
        self.thread_id = thread_id

    async def interaction_check(self, interaction):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "❌ Esta decisão pertence ao jogador inscrito.",
                ephemeral=True
            )
            return False
        return True

    async def _enviar_log_cancelamento(self, interaction, tipo, thread):
        jogo = getattr(thread, "name", f"Thread {self.thread_id}")
        if tipo == 'creditos':
            await enviar_log_bot(
                interaction.guild,
    ...
    async def _cancelar(self, interaction, tipo):
        ...
        try:
            cur.execute(
                "SELECT estado, metodo_pagamento FROM inscricoes_jogos WHERE id=%s FOR UPDATE",
                (self.inscricao_id,)
            )
            row = cur.fetchone()
            if not row:
                conn.rollback()
                return await interaction.followup.send("⚠️ Inscrição não encontrada.", ephemeral=True)

            estado_atual, metodo_pagamento = row

            # Recuperação idempotente: se a operação anterior foi gravada mas a resposta/log falhou,
            ...
            if estado_atual == 'cancelado':
                ...
                return await interaction.followup.send(
                    "⚠️ A inscrição já não está validada e não pode ser cancelada por este fluxo.",
                    ephemeral=True
                )
            else:
                cur.execute(
                    "UPDATE inscricoes_jogos SET estado='cancelado' WHERE id=%s AND estado='pago'",
                    (self.inscricao_id,)
                )
                if cur.rowcount != 1:
                    conn.rollback()
                    return await interaction.followup.send(
                        "⚠️ A inscrição foi alterada entretanto. Tenta novamente.",
                        ephemeral=True
                    )
                ...
                if metodo_pagamento == 'economia':
                    ...
                    movimentar_creditos_cursor(
                        cur,
                        self.user_id,
                        CREDITOS_CANCELAMENTO_VALIDADO,
            ...
            conn.rollback()
            print(f"Erro cancelamento validado: {e}")
            return await interaction.followup.send(
                "⚠️ Não foi possível cancelar a inscrição.",
                ephemeral=True
            )
        finally:
            cur.close()
        ...
        if pedido_existente:
            tipo_final = pedido_existente[0]
            await self._enviar_log_cancelamento(interaction, tipo_final, thread)
            for item in self.children:
                item.disabled = True
            try:
            ...
            estado_pedido = pedido_existente[1]
            if tipo_final == 'creditos':
                texto = "ℹ️ Esta inscrição já tinha sido cancelada e os créditos já foram processados. O log foi reenviado à staff."
            else:
                texto = f"ℹ️ Esta inscrição já tinha sido cancelada. O pedido de devolução está **{estado_pedido}** e o alerta foi reenviado à staff."
            return await interaction.followup.send(texto, ephemeral=True)
        ...
        if thread:
            try:
                ...
                print(f"Erro ao atualizar mensagem após cancelamento: {e}")

        # O log é enviado antes da mensagem final ao jogador. Mesmo que a edição do embed falhe,
        # a staff continua a receber o alerta.
        await self._enviar_log_cancelamento(interaction, tipo_final, thread)
        ...
        texto = (
            f"✅ Inscrição cancelada e **{CREDITOS_CANCELAMENTO_VALIDADO} créditos** adicionados à carteira."
            if tipo_final == 'creditos'
    ...
    async def creditos(self, interaction, button):
        await self._cancelar(interaction, 'creditos')
    ...
    async def voltar(self, interaction, button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(
            content="Cancelamento interrompido.",
            embed=None,
            view=self
        )
