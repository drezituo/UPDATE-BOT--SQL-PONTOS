...
def _nfc_minigame_barra_html(team: str, action: str, codigo_lido: str, token: str):
    """Mini-game SATCOM: barra de sincronização. Não mostra o código no ecrã."""
    team = str(team or "").strip().lower()
    action = str(action or "codigo").strip().lower()
    codigo_lido = str(codigo_lido or "").strip().upper()
    token = str(token or "")
    ...
    operation_state = "ATIVA" if active else "INATIVA"
    success_url = (
        f"/api/nfc/milsim/tap?action={action}"
        f"&team={team}"
...
async def nfc_minigame_barra(request: web.Request):
    if not _nfc_authorized(request):
        return _nfc_html(False, "Não autorizado.", status=401)

    team = request.query.get("team", "")
    action = request.query.get("action", "codigo")
    codigo_lido = request.query.get("codigo", "")
    token = request.query.get("token", "")

    if str(action or "").strip().lower() != "codigo" or str(codigo_lido or "").strip().upper() != "BUNKER-551":
        return _nfc_html(False, "Mini-game não autorizado para este objetivo.", status=403, team=team, action=action)

    return _nfc_minigame_barra_html(team, action, codigo_lido, token)
...
async def start_nfc_api_server():
    ...
    app_api.router.add_get("/api/nfc/minigame/barra", nfc_minigame_barra)
