...
async def codigo(ctx, codigo: str):
    ...
    if codigo in team_state["completed_codes"]:
        # Proteção anti-bug: se o código ficou marcado como usado mas a equipa ainda está
        # na mesma missão/fase operacional, permitimos uma nova validação para não queimar
        # objetivos por erro de ordem lógica, NFC repetido ou validação interrompida.
        if (
            team_state.get("phase") == "mission"
            and team_state.get("current") == data.get("mission")
            and data.get("type") in ("complete", "end")
        ):
            try:
                team_state["completed_codes"].remove(codigo)
            except ValueError:
                pass
            await milsim_log(
                f"♻️ Código `{codigo}` estava marcado como usado por **{team.upper()}**, "
                "mas a missão ainda estava ativa. Validação reaberta automaticamente."
            )
        else:
            return await ctx.send(
                "✅ Objetivo já validado. Aguardem reagrupamento ou novas ordens do Comando Central."
            )
    ...
    # RAVEN-119 só pode ficar marcado como usado se a sabotagem for realmente aplicada.
    # Antes, uma tentativa inválida podia queimar o código sem concluir o objetivo.
    if codigo == "RAVEN-119":
